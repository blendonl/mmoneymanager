...
const paddleocr_http_service_1 = require("./paddleocr-http.service");
describe('PaddleOcrHttpService', () => {
    ...
    beforeEach(() => {
        ...
        service = new paddleocr_http_service_1.PaddleOcrHttpService(configService);
    ...
    describe('extractText', () => {
        ...
        it('should handle service errors', async () => {
            ...
            const imageBuffer = Buffer.from('fake-image-data');
            await expect(service.extractText(imageBuffer)).rejects.toThrow(/PaddleOCR service error/);
        });
        ...
        it('should fail after max retries', async () => {
            ...
            const imageBuffer = Buffer.from('fake-image-data');
            await expect(service.extractText(imageBuffer)).rejects.toThrow(/Failed to extract text from image after 3 attempts/);
            expect(global.fetch).toHaveBeenCalledTimes(3);
        });
        ...
        it('should use correct service URL from config', async () => {
            const customConfigService = {
                get: jest.fn((key, defaultValue) => {
                    if (key === 'PADDLEOCR_SERVICE_URL')
                        return 'http://custom:9000';
                    if (key === 'PADDLEOCR_TIMEOUT')
                        return 5000;
                    return defaultValue;
                }),
            };
            const customService = new paddleocr_http_service_1.PaddleOcrHttpService(customConfigService);
            global.fetch = jest.fn().mockResolvedValue({
                ok: true,
            ...
            expect(global.fetch).toHaveBeenCalledWith('http://custom:9000/ocr/extract', expect.any(Object));
        });
    });
});
//# sourceMappingURL=paddleocr-http.service.spec.js.map